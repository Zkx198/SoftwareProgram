...
// 界面开始！！！
	public void cal() {
		cal_Frame = new Frame("\u4E8C\u5143\u4E00\u6B21\u65B9\u7A0B\u6C42\u89E3\u8BA1\u7B97\u5668");
...
// 功能开始！！！
	public void actionPerformed(ActionEvent e) {
		...
		/*
		 * // key "=" if (this.keyAvailable && e.getActionCommand() == "=") {
		 * this.alreadyClickedEqual = true; switch (this.operator) { case 1: this.result
		 * += this.inputContent; break; case 2: this.result -= this.inputContent; break;
		 * case 3: this.result *= this.inputContent; break; case 4: { if
		 * (this.inputContent == 0) { display_Textfield.setText("除数不能为零!");
		 * this.keyAvailable = false; } else this.result = this.result /
		 * this.inputContent; } default: display_Textfield.setText("default跳转调试1"); } if
		 * (this.combi == 1) { this.combi = 0; if (this.inputContent <= 0 || this.result
		 * <= 0 || this.inputContent != (int) this.inputContent || this.result != (int)
		 * this.result) { display_Textfield.setText("请输入正整数！"); this.keyAvailable =
		 * false; } else { this.result = Combinatorial((int) this.result, (int)
		 * this.inputContent); } } if (this.keyAvailable)
